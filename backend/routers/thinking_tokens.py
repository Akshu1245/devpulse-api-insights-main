...
from services.thinking_tokens import extract_thinking_tokens_from_usage, aggregate_thinking_token_stats
...
def get_thinking_token_stats(
    ...
    return {
        ...
        "recent_anomalies": [
            r for r in records[:50] if r.get("is_thinking_anomaly")
        ][:10],
    }
...
def analyze_thinking_efficiency(
    ...
    top_endpoint = max(by_endpoint.items(), key=lambda x: x[1]) if by_endpoint else ("unknown", 0)
    ...
    if total_thinking_cost > total_cost * 0.3:
        recommendations.append({
            ...
            "message": f"Thinking tokens account for {round(total_thinking_cost/total_cost*100, 1)}% of your LLM costs. Add max_reasoning_tokens limits to your API calls.",
    ...
    if top_pct > 50:
        recommendations.append({
            ...
            "message": f"Your '{top_endpoint[0]}' endpoint accounts for {top_pct}% of total LLM costs. Optimize this endpoint first.",
    ...
    if anomaly_count > 0:
        recommendations.append({
            ...
            "message": f"{anomaly_count} LLM calls detected with abnormal thinking token usage. These may indicate runaway reasoning loops.",
    ...
    return {
        ...
        "thinking_cost_pct": round(total_thinking_cost / total_cost * 100, 1) if total_cost > 0 else 0,
