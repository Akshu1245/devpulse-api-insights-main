...
THINKING_TOKEN_MODELS = {
    ...
    "o1": {"provider": "openai", "thinking_field": "reasoning_tokens", "multiplier_range": (10, 30)},
    "o1-mini": {"provider": "openai", "thinking_field": "reasoning_tokens", "multiplier_range": (5, 15)},
    "o1-preview": {"provider": "openai", "thinking_field": "reasoning_tokens", "multiplier_range": (10, 30)},
    "o3": {"provider": "openai", "thinking_field": "reasoning_tokens", "multiplier_range": (15, 40)},
    "o3-mini": {"provider": "openai", "thinking_field": "reasoning_tokens", "multiplier_range": (8, 20)},
    "o4-mini": {"provider": "openai", "thinking_field": "reasoning_tokens", "multiplier_range": (8, 20)},
    # Anthropic extended thinking models
    "claude-3-7-sonnet": {"provider": "anthropic", "thinking_field": "thinking_tokens", "multiplier_range": (5, 20)},
    "claude-3-5-sonnet": {"provider": "anthropic", "thinking_field": "thinking_tokens", "multiplier_range": (3, 10)},
    "claude-opus-4": {"provider": "anthropic", "thinking_field": "thinking_tokens", "multiplier_range": (10, 30)},
    # Google Gemini thinking
    "gemini-2.0-flash-thinking": {"provider": "google", "thinking_field": "thinking_tokens", "multiplier_range": (5, 15)},
}
...
TOKEN_COSTS_USD = {
    ...
    "gemini-2.0-flash-thinking": {"input": 0.0001, "output": 0.0004, "thinking": 0.0035},
...
def extract_thinking_tokens_from_usage(
    ...
    costs = TOKEN_COSTS_USD.get(normalized_model, {"input": 0.001, "output": 0.002, "thinking": 0.0})
    ...
    # Anthropic format: usage.cache_read_input_tokens or thinking block
    ...
    if thinking_tokens == 0:
        ...
        prompt = int(usage_metadata.get("prompt_tokens", 0) or usage_metadata.get("input_tokens", 0))
        completion = int(usage_metadata.get("completion_tokens", 0) or usage_metadata.get("output_tokens", 0))
    ...
    output_tokens = int(usage_metadata.get("completion_tokens", 0) or usage_metadata.get("output_tokens", 0))
    if response_latency_ms > 0 and output_tokens > 0:
        ...
        if ms_per_output_token > 30:
            ...
            if thinking_tokens == 0 and model_info:
                ...
                multiplier_min, multiplier_max = model_info.get("multiplier_range", (5, 15))
    ...
    # Calculate costs
    input_tokens = int(usage_metadata.get("prompt_tokens", 0) or usage_metadata.get("input_tokens", 0))
    ...
    is_thinking_anomaly = thinking_tokens > (output_tokens * 3) if output_tokens > 0 else False
    ...
    return {
        ...
        "ms_per_output_token": round(response_latency_ms / max(output_tokens, 1), 1) if output_tokens > 0 else 0,
...
def aggregate_thinking_token_stats(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    total_cost_inr = sum(float(r.get("cost_inr", 0)) for r in records)
    cost_without_thinking = total_cost_inr - total_thinking_cost_inr

    avg_overhead = (
        total_cost_inr / cost_without_thinking
        if cost_without_thinking > 0
        else 1.0
    )

    anomaly_calls = sum(
        1 for r in records
        if int(r.get("thinking_tokens", 0)) > int(r.get("tokens_used", 1)) * 3
    )
    ...
    return {
        ...
        "potential_savings_inr": round(total_thinking_cost_inr * 0.5, 4),  # 50% savings with optimization
    }
Thinking Token Cost Attribution — DevPulse Patent 2
Detects, attributes, and classifies thinking token expenditure in LLM API calls
using differential token analysis and response timing signatures.
