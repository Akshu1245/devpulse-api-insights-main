...
import { decryptKey } from "../_shared/key-crypto.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isAllowedHost(parsedUrl.hostname)) {
      return new Response(JSON.stringify({ error: "Host not allowed (private/internal hosts are blocked)" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (apiKeyId) {
      ...
      if (user?.id) {
        ...
        if (keyData?.encrypted_key && secret) {
          apiKey = await decryptKey(keyData.encrypted_key, secret);
        }
      }
    }
    ...
    if (apiKey) {
      ...
      } else if (updatedUrl.searchParams.has("apiKey")) {
        updatedUrl.searchParams.set("apiKey", apiKey);
      } else if (updatedUrl.searchParams.has("apikey")) {
        updatedUrl.searchParams.set("apikey", apiKey);
