...
/* ── Radar ping rings (GPU-accelerated) ── */
const RadarRings = memo(function RadarRings({ active }: { active: boolean }) {
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
      {[1, 2, 3, 4].map((i) => (
        <motion.div
          ...
          animate={
            active
              ? { scale: [1, 1.12, 1], opacity: [0.15, 0.4, 0.15] }
              : { scale: 1, opacity: 0.08 }
          }
          transition={
            active
              ? { duration: 1.2, delay: i * 0.15, repeat: Infinity, ease: "easeInOut" }
              : { duration: 0.3 }
          }
        />
      ))}
...
/* ── Scanning sweep line ── */
const ScanSweep = memo(function ScanSweep({ active }: { active: boolean }) {
...
const discoveredAPIs = [
  { name: "Stripe", angle: 30, dist: 140, delay: 0.1 },
  { name: "GitHub", angle: 95, dist: 170, delay: 0.2 },
  { name: "OpenAI", angle: 160, dist: 130, delay: 0.3 },
  { name: "AWS", angle: 230, dist: 155, delay: 0.4 },
  { name: "Twilio", angle: 310, dist: 145, delay: 0.5 },
];

const DiscoveredNode = memo(function DiscoveredNode({
  api,
  visible,
}: {
  api: (typeof discoveredAPIs)[0];
  visible: boolean;
}) {
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          transition={{ delay: api.delay, duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
        >
  ...
  return (
    ...
    >
      ...
      <svg width="120" height="120" viewBox="0 0 120 120" className="absolute -inset-[10px]">
        <circle cx="60" cy="60" r="52" fill="none" stroke="hsl(var(--muted) / 0.3)" strokeWidth="3" />
        <motion.circle
          cx="60"
          cy="60"
          r="52"
...
/* ── Boot log terminal (ultra-fast) ── */
const bootMessages = [
  { text: "> Initializing DevPulse core...", delay: 0 },
  { text: "> Loading probe modules...", delay: 60 },
  { text: "> Scanning API endpoints...", delay: 120 },
  { text: "> Stripe .......... ONLINE", delay: 180, status: "ok" },
  { text: "> GitHub .......... ONLINE", delay: 230, status: "ok" },
  { text: "> OpenAI .......... DEGRADED", delay: 280, status: "warn" },
  { text: "> AWS S3 .......... ONLINE", delay: 330, status: "ok" },
  { text: "> Twilio .......... ONLINE", delay: 380, status: "ok" },
  { text: "> All systems operational", delay: 430, status: "ok" },
  { text: "> Launching dashboard...", delay: 480 },
];

const BootLog = memo(function BootLog({ active }: { active: boolean }) {
...
export default function SplashScreen({ onComplete }: { onComplete: () => void }) {
  ...
  const startHold = useCallback(() => {
    ...
    setPhase("holding");
    let p = 0;
    // Ultra-fast fill: 20ms interval, 10% per tick = ~200ms to fill
    holdInterval.current = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        setPhase("scanning");
        setShowNodes(true);
        // Show boot log immediately
        setTimeout(() => setShowBoot(true), 50);
        // Complete after boot messages finish (~550ms)
  ...
  useEffect(() => {
    const timeoutId = setTimeout(startHold, 100);
