...
export default function ComplianceReportPanel() {
  ...
  const downloadPdfReport = async () => {
    if (!report) return;
    try {
      const response = await fetch(`/api/compliance/report/${userId}/pdf?organization=${encodeURIComponent(report.organization)}&report_type=both`);
      if (!response.ok) {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `devpulse-compliance-${report.report_id}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (error) {
