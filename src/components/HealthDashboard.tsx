...
export default function HealthDashboard() {
  ...
  const getUserKeyMap = useCallback(async (): Promise<Record<string, string>> => {
    if (!user) {
    ...
    if (error || !data?.keyMap) {
      return {};
    }

    return data.keyMap as Record<string, string>;
  }, [user, apiKeys]);
      ...
      const previewPromises = activeApis.slice(0, 6).map(async (api) => {
        try {
          ...
          if (api.requiresKey && keyMap[api.id]) {
            url = url.replace(/apikey=[^&]+/, `apikey=${encodeURIComponent(keyMap[api.id])}`);
            url = url.replace(/api_key=[^&]+/, `api_key=${encodeURIComponent(keyMap[api.id])}`);
            url = url.replace(/appid=[^&]+/, `appid=${encodeURIComponent(keyMap[api.id])}`);
            url = url.replace(/key=[^&]+/, `key=${encodeURIComponent(keyMap[api.id])}`);
          }
